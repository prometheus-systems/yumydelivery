...
@Component({ 
  selector: 'app-filtrorel-grupo_usuarios', 
  templateUrl: './filtrorel-grupo_usuarios.component.html', 
}) 
export class Filtrorelgrupo_usuariosComponent implements OnInit { 
  ...
  onSubmit(form: NgForm) { //ENVIANDO DADOS DO FORM PARA SERVICE CHAMAR A FUNCAO PARA GRAVAR NO BACKEND 
    if (this.validateForm()) { 
      this.apiService.postRelatoriogrupo_usuarios() 
        .pipe(first()) 
        .subscribe( 
           dados => { 
