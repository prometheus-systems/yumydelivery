...
export class relatoriovendedoresComponent implements OnInit { bounce:any;
  ...
  Voltar(){ 
    this.router.navigate(['Listarvendedores'], {skipLocationChange: true, fragment: 'top'}); 
  ...
  populaDadosChart(){ 
    ...
    for (let i=0;i<this.apiService.arrvendedores.length;i++){ 
      if ((vGrupo!=this.apiService.arrvendedores[i].ven_nome)&&(vGrupo)){ 
        registro={'grupo':vGrupo}; 
        this.arrGrupo.push(registro); 
        this.apiService.DataChart.push(registro); 
      }else{ 
      }  
      vGrupo=this.apiService.arrvendedores[i].ven_nome;  
    } 
    registro={'grupo':vGrupo};  
    this.apiService.DataChart.push(registro); 
    this.arrGrupo.push(registro); 
  ...
  GoogleChart(){ 
    this.router.navigate(['Graficovendedores'], {skipLocationChange: true, fragment: 'top'});  
  } 
