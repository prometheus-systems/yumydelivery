...
@Component({ 
  selector: 'app-filtrorel-lista_pedidos', 
  templateUrl: './filtrorel-lista_pedidos.component.html', 
  animations: [trigger('bounce', [transition('* => *', useAnimation(fadeInUp,{params: {timing: 0.2,}}))])] 
}) 
export class Filtrorellista_pedidosComponent implements OnInit { bounce:any;
...
, private apicategorias:categoriasService
  ...
  onSubmit(form: NgForm) { //ENVIANDO DADOS DO FORM para SERVICE CHAMAR A FUNCAO para FILTRO DO RELATORIO 
    ...
    if (this.validateForm()) { 
      this.apiService.postRelatoriolista_pedidos() 
        .pipe(first()) 
        .subscribe( 
           dados => { 
           if (Array.isArray(dados) && dados.length){ 
              console.log('relatorio',dados);  
              this.apiService.arrlista_pedidos = dados; 
              ...
              if (Array.isArray(this.apiService.arrlista_pedidos) && this.apiService.arrlista_pedidos.length){  
