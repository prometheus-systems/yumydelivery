...
@Component({ 
  selector: 'app-filtrorel-endereco', 
  templateUrl: './filtrorel-endereco.component.html', 
  animations: [trigger('bounce', [transition('* => *', useAnimation(fadeInUp,{params: {timing: 0.2,}}))])] 
}) 
export class FiltrorelenderecoComponent implements OnInit { bounce:any;
  ...
  onSubmit(form: NgForm) { //ENVIANDO DADOS DO FORM para SERVICE CHAMAR A FUNCAO para FILTRO DO RELATORIO 
    if (this.validateForm()) { 
      this.apiService.postRelatorioendereco() 
        .pipe(first()) 
        .subscribe( 
           dados => { 
