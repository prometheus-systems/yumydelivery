...
@Component({ 
  selector: 'app-grafico-bonificacao', 
  templateUrl: 'grafico-bonificacao.component.html',  
  animations: [trigger('bounce', [transition('* => *', useAnimation(fadeInUp,{params: {timing: 0.2,}}))])] 
}) 

export class GraficobonificacaoComponent  { bounce:any; 
   Device = environment.Device; 
   constructor(public apiService: bonificacaoService
     ,private _translate: TranslateService 
     ,private router: Router)  { } 
     title = 'GRÁFICO DE BONIFICAÇÃO '; 
     type = 'PieChart'; 
     data = this.getData(); 
     columnNames = ['Vendedores', 'total']; 
     options = { 
     };  
     width = 700; 
     height = 700; 
   ...
   getChart(){ 
      ...
      this.columnNames = ['Vendedores', 'total']; 
      this.options = {  
      };  
      this.width = 700; 
      this.height = 700; 
